(function(){
  ...
  function addMath(DATA) {
    const M = "math";
    [
      ["大数的认识","亿以内数的读写",3,"5080000 读作什么？","五百零八万",["五十万八千","五百八十万","五千零八十"],"5080000 写成 508 万，万级读作五百零八，个级全为 0 不读，所以是五百零八万。"],
  ...
  function addScience(DATA) {
    ...
    [
      ...
      ["实验安全","安全判断",1,"发现没有标签、气味不明的实验材料时应该怎么做？","不要直接闻或尝，请老师确认",["鼻子凑近猛闻","直接尝一口","自行加热后再闻"],"未知材料可能有危险；不要自行接触、闻或尝，先请老师确认。"]
  ...
  function addGeo(DATA) {
    ...
    COUNTRY_CAPITALS.forEach(([country,cap,cont],i)=>{
      addQ(DATA,"geography","世界国家与首都","世界首都",2,country==="南非"?"南非的行政首都是哪里？":`${country}的首都是哪里？`,cap,pickWrongs(allCountryCaps, cap, i),country==="南非"?"南非的行政首都是比勒陀利亚；开普敦是立法首都，布隆方丹是司法首都。":`${country}的首都是${cap}。注意不要把著名城市误认为首都。`,{courseType:"extension"});
    });
